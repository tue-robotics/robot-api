...
class Body {
  ...
  constructor(robot) {
    ...
    parameter.get(parameter_ => {
      ...
      parameter_.arm.joint_names.forEach(jointName => {
        that.jointNames.arm.push(`${jointName}`);
        that.jointNames.leftArm.push(`${jointName}_left`);
        that.jointNames.rightArm.push(`${jointName}_right`);
      });
      // Torso joint name
      that.jointNames.torso = parameter_.torso.joint_names;

      // Default joint configuration. N.B., the configurations for the left
      // and right arm are equal
      that.defaultConfigurations = {};
      that.defaultConfigurations.torso = parameter_.torso.default_configurations;
      that.defaultConfigurations.arm = parameter_.arm.default_configurations;
      that.defaultConfigurations.leftArm = parameter_.arm.default_configurations;
      that.defaultConfigurations.rightArm = parameter_.arm.default_configurations;
    });
