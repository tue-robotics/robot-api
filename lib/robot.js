...
// Private variables
// location is only available in browsers and web workers, not in Node
const search = globalThis.location?.search ?? '';
const host = queryString.parse(search).host || hostname() || 'localhost';
...
class Robot extends EventEmitter2 {
  ...
  /**
   ...
   are provided.
   @param {string} [url] The rosbridge websocket URL to connect to
   */
  connect(url) {
